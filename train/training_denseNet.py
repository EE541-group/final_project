...
def build_densenet(num_classes, backbone="121", pretrained=True):
    fn = getattr(models, f"densenet{backbone}")
    model = fn(pretrained=pretrained)
...
OUTPUT_ROOT    = "./checkpoint/denseNet"
        ...
        for opt_name, opt_fn in OPTIMIZERS.items():
            for strat_key, strat in STRATEGIES.items():
                ...
                if kind == "resnet":
                    ...
                    model = build_densenet(NUM_CLASSES, backbone=name).to(DEVICE)
...
for exp, hist in results.items():
    ...
    plt.title(f"{exp} Loss"); plt.xlabel('Epoch'); plt.ylabel('Loss'); plt.legend()
    plt.subplot(1, 2, 2)
    plt.plot(epochs, hist['train_acc'], label='Train Acc')
    plt.plot(epochs, hist['val_acc'], label='Val Acc')
    plt.title(f"{exp} Accuracy"); plt.xlabel('Epoch'); plt.ylabel('Accuracy'); plt.legend()
    plt.tight_layout()
    plt.show()
